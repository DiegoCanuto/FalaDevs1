...
public class Locked_Door : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if(Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, 5f))
		{
			...
			}else if (hit.collider.CompareTag("Door") && haveKey && locked)// Validar se o player tem a chave e se a porta está trancada
			{
				CancelInvoke("DesactivateLabel");
				txt_DoorState.text = "<b><Color=Lime>Usar Chave!</Color> \n" +
				"<Color=Magenta>Aperte 'E' para abrir a porta.</Color></b>";

				// Abre a porta e usa chave
				if (Input.GetKeyDown("e"))	{					
					
					locked = false;
					Door_To_Open.GetComponent<Animator>().enabled = true;
